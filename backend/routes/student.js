...
router.post('/api/students', mongoChecker, authenticate, async (req, res) => {
	// log(req.body)
...
router.get('/api/students', mongoChecker, authenticate, async (req, res) => {
...
router.get('/api/students/:id', mongoChecker, authenticate, async (req, res) => {
	...
	try {
		const student = await Student.findOne({_id: id, creator: req.user._id})
		if (!student) {
...
router.delete('/api/students/:id', mongoChecker, authenticate, async (req, res) => {
	const id = req.params.id
	...
	try {
		const student = await Student.findOneAndRemove({_id: id, creator: req.user._id})
		if (!student) {
			res.status(404).send()
		} else {   
			res.send(student)
...
router.put('/api/students/:id', mongoChecker, authenticate, async (req, res) => {
	const id = req.params.id
	...
	try {
		const student = await Student.findOneAndReplace({_id: id, creator: req.user._id}, req.body, {new: true})
		if (!student) {
			res.status(404).send()
		} else {   
			res.send(student)
...
router.patch('/api/students/:id', mongoChecker, authenticate, async (req, res) => {
	const id = req.params.id
	...
	try {
		const student = await Student.findOneAndUpdate({_id: id, creator: req.user._id}, {$set: fieldsToUpdate}, {new: true})
		if (!student) {
			res.status(404).send('Resource not found')
		} else {   
			res.send(student)
